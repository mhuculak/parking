...
public class SignRecognizer {
	...
	public SignRecognizer(File file, Logger logger) {
		m_logger = new Logger(logger, this, LoggingTag.Image);
		origImage = new SignImage(file, m_logger);
		loadSign();
	}

	public SignRecognizer(SignImage img, Logger logger) {
		origImage = img;
		m_logger = logger;
		loadSign();
	}

	public SignRecognizer(InputStream inStream, Logger logger, Profiler profiler) throws Exception {
		m_profiler = profiler;
		m_logger = logger;
...
//		rectangles = Rectangle.getRectanglesFromLines(verifiedLines, workingImage.getMinDim());
		...
		if (signImages.size() > 0) {
			...
			logger.error("no sign recognized");
		}		
