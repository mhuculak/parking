...
public class SignPictures {
    ...
    private static final String picCountFile = "pictureCount";
    private static final String locationtDir = "parking/location";
    ...
    public static void saveLocationData(Context context, File filesDir, int num, List<Position> points) {
        if (points == null || points.size()==0) {
            return;
        }
        try {
            File locDir = new File(filesDir+File.separator+locationtDir);
            if (!locDir.exists()) {
                if (locDir.mkdirs()) {
