...
//       		System.out.println("display outerEllipse "+outerEllipse);
    ...
    public static List<SignImage> getSigns(SignImage image, SignImage canny, List<Rectangle> rectangles, boolean findEllipse, Logger logger) {
    	List<SignImage> signs = new ArrayList<SignImage>();    	
    	for ( int i=0 ; i<rectangles.size() && i<maxSigns; i++) {			
    		SignImage sign = createSign(image, canny, rectangles.get(i), findEllipse, logger);    		
    		signs.add(sign);
    	}
...
//		System.out.println("full size is "+full.size()+" reduced is "+reduced.size());
    ...
    public static SignImage createSign(SignImage image, SignImage canny, Rectangle rect, boolean findEllipse, Logger logger) {
